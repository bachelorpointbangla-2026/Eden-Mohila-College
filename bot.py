...
# ----------- ১. ফ্লাস্ক সার্ভার (বট ২৪/৭ লাইভ রাখার জন্য) -----------
app = Flask('')

@app.route('/')
def home():
    return "Scanner Bot Online!"
...
user_context = {}    # Sonali রেজাল্টের জন্য current_end ট্র্যাক করতে

# =======================================================
#               ১ম কোড: 1️⃣ Sonali Result
# =======================================================
...
def run_search(chat_id, s_r, e_r):
    ...
    user_context[chat_id] = {"current_end": e_r}
    found_students = 0
    ...
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("👉 Next 500?", callback_data="next_500"))
    bot.send_message(chat_id, f"✅ Done!\n📊 Total Found: {found_students}", reply_markup=markup)
    active_tasks[chat_id] = False


# =======================================================
#               ২য় কোড: 2️⃣ eShiksha Result
# =======================================================

BASE_URL = "https://edencollege.eshiksabd.com"
...
def get_session():
    ...
    login_payload = {
        'username': 'edenstudent',
        'password': 'edenstudent',
        'submit': 'Sign In'
    }
    try:
...
def fetch_single(session, roll):
    try:
        ...
        for img in soup.find_all('img'):
            src = img.get('src', '')
            if 'image/student' in src.lower() or 'student' in src.lower():
...
def create_action_keyboard(phone, next_start=None, next_end=None):
    ...
    if next_start and next_end:
        markup.add(InlineKeyboardButton("➡️ Next 500", callback_data=f"next_{next_start}_{next_end}"))
...
def run_range_search(chat_id, start_roll, end_roll):
    ...
    next_markup.add(InlineKeyboardButton("➡️ Next 500", callback_data=f"next_{next_start}_{next_end}"))
...
def send_welcome(message):
    bot.send_message(
        message.chat.id,
        "👋 **কোন অপশনটি ব্যবহার করতে চান সিলেক্ট করুন:**",
        parse_mode='Markdown',
...
def handle_callbacks(call):
    ...
    if call.data == "mode_sonali":
        ...
        bot.answer_callback_query(call.id, "1️⃣ Sonali Result নির্বাচিত!")
        # ১ম কোডের আসল স্টার্ট রেসপন্স
        markup = InlineKeyboardMarkup([[InlineKeyboardButton("Start Search", callback_data="btn_ready")]])
        bot.send_message(chat_id, "7 College Payment Scanner!", reply_markup=markup)

    elif call.data == "mode_eshiksha":
        user_modes[chat_id] = "eshiksha"
        bot.answer_callback_query(call.id, "2️⃣ eShiksha Result নির্বাচিত!")
        # ২য় কোডের আসল স্টার্ট রেসপন্স
        bot.send_message(
            chat_id,
            "🏛️ *ইডেন মহিলা কলেজ বট*\n\n"
            "• রোল নম্বর দিন (যেমন: `2202324260277`)\n"
            "• অথবা রেঞ্জ দিন (যেমন: `2202324260270-2202324260320`)",
            parse_mode='Markdown'
        )

    # ১ম কোডের বাটন হ্যান্ডলিং
    elif call.data == "btn_ready":
        bot.answer_callback_query(call.id)
        bot.send_message(chat_id, "🚀 রোল বা রেঞ্জ পাঠান।")

    elif call.data == "next_500":
        bot.answer_callback_query(call.id)
        le = user_context.get(chat_id, {}).get("current_end", 0)
        if le > 0:
            run_search(chat_id, le + 1, le + 500)

    # ২য় কোডের বাটন হ্যান্ডলিং
    ...
    if mode == "sonali":
        try:
            if "-" in text:
                ...
                run_search(chat_id, s, e)
            else:
                run_search(chat_id, int(text), int(text))
        except:
            pass
